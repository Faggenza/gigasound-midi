...
{
    ggl_clear_fb(fb);
    ggl_draw_sprite(fb, 0, 0, menu_side_sprite);

    int start_y = 2 + state->old_selection * (menu_selected_sprite.height + 1);
    int end_y = 2 + state->selected * (menu_selected_sprite.height + 1);
    ...
    ggl_draw_sprite(fb, menu_side_sprite.stride, y, menu_selected_sprite);
    ...
    {
        int icon_x = 100;
        int icon_y = 1 + i * (menu_selected_sprite.height + 1);
